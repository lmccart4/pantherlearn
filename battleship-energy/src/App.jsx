...
export default function App() {
  ...
  const handleGameOver = async (stats) => {
    ...
    reportScore("battleship-energy", stats.totalScore, 5000, {
      grade: tier.percent,
    ...
    try {
      ...
      if (urlCourseId) {
        const gradeRef = doc(db, "progress", user.uid, "courses", urlCourseId, "activities", "battleship-energy");
        const reviewRef = doc(db, "courses", urlCourseId, "battleshipEnergy", user.uid);
        ...
        if (tier.value > existingScore) {
          // Write auto-grade to progress/{uid}/courses/{courseId}/activities/battleship-energy
          await setDoc(gradeRef, {
            activityScore: tier.value,
            activityLabel: tier.label,
            activityType: "battleship-energy",
            activityTitle: "Battleship Energy Quiz",
            gradedAt: serverTimestamp(),
          }, { merge: true });

          // Write raw data to courses/{courseId}/battleshipEnergy/{uid} for teacher review
