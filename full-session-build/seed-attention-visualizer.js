// seed-attention-visualizer.js
// Seeds the Attention Visualizer lesson into PantherLearn
// Run from your pantherlearn directory: node seed-attention-visualizer.js
...
const lesson = {
  title: "Seeing Attention in Action: The Attention Visualizer",
  ...
  blocks: [
    ...
    },
    {
      ...
      placeholder: "In sentence 1, the words ___ and ___ tell me bank means... In sentence 2..."
    },
    {
      id: "b3",
      type: "callout",
    ...
    },
    {
      id: "b7",
      type: "embed",
      url: "https://attention-visualizer.web.app/?studentId={{studentId}}&courseId={{courseId}}&blockId=b7&lessonId=lesson-11-attention-visualizer",
    ...
    },
    {
      ...
      terms: [
        { term: "Attention Weight", definition: "A number (0% to 100%) showing how much one word 'pays attention to' another when determining meaning." },
        { term: "Context Window", definition: "All the words the model can 'see' at once when making predictions. Longer context windows let the model consider more surrounding text." },
        { term: "Disambiguation", definition: "The process of determining which meaning of a word is intended based on context. Example: 'bank' → financial institution vs. edge of a river." },
        { term: "Positional Encoding", definition: "How the model tracks word ORDER in a sentence. 'The patient doctor' is different from 'the doctor's patient' because position matters." }
      ]
    }
  ]
};
...
async function seed() {
  // ⬇️ UPDATE THESE COURSE IDs ⬇️
  const courses = [
    { courseId: "PERIOD_4_COURSE_ID", lessonId: "lesson-11-attention-visualizer" },
    { courseId: "PERIOD_5_COURSE_ID", lessonId: "lesson-11-attention-visualizer" },
    { courseId: "PERIOD_7_COURSE_ID", lessonId: "lesson-11-attention-visualizer" },
  ];

  for (const { courseId, lessonId } of courses) {
    await setDoc(doc(db, "courses", courseId, "lessons", lessonId), lesson);
    console.log(`✅ Seeded Attention Visualizer for course ${courseId}`);
  }
  ...
  console.log(JSON.stringify({
    ...
    icon: "🧠",
    collection: "attention_visualizer",
    url: "https://attention-visualizer.web.app",
    course: "ai-literacy",
    maxScore: 100,
  }, null, 2));
