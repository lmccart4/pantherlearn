// src/components/grading/activities/NeuralNetworkLabReview.jsx
// Review component for the external Neural Network Lab activity.
// Reads from root-level "neural_network_lab" collection in Firestore.
...
const STAGE_META = [
  { key: "stage1", name: "Wire the Network", icon: "🔌" },
  { key: "stage2", name: "Automated Training", icon: "⚙️" },
  { key: "stage3", name: "Test & Classify", icon: "🧪" },
  { key: "stage4", name: "Real-World & Bias", icon: "🌍" },
];

export default function NeuralNetworkLabReview({ activity, studentMap, courseId: parentCourseId }) {
  ...
  useEffect(() => {
    const fetchData = async () => {
      setLoading(true);
      try {
        const snap = await getDocs(collection(db, "neural_network_lab"));
        ...
        await Promise.all(subs.map(async (sub) => {
          ...
          for (const cid of courseIdsToCheck) {
            try {
              const gradeDoc = await getDoc(doc(db, "progress", uid, "courses", cid, "activities", "neural-network-lab"));
              if (gradeDoc.exists()) {
                existingGrades[uid] = {
        ...
        console.error("Error fetching Neural Network Lab data:", err);
      }
      setLoading(false);
    };
    fetchData();
  ...
  const handleGrade = async (uid, tier) => {
    ...
    try {
      ...
      const gradeRef = doc(db, "progress", uid, "courses", courseId, "activities", "neural-network-lab");
      await setDoc(gradeRef, {
        activityScore: tier.value,
        activityLabel: tier.label,
        activityType: "neural-network-lab",
        activityTitle: "Neural Network Lab",
        gradedAt: new Date(),
      }, { merge: true });
      ...
      try {
        ...
        if (xpAmount > 0) await awardXP(uid, xpAmount, `activity_grade:neural-network-lab:${tier.label.toLowerCase()}`, courseId);
      } catch (xpErr) { console.warn("Could not award activity XP:", xpErr); }

      try {
        await createNotification(uid, {
          type: "grade_result",
          title: `Neural Network Lab graded: ${tier.label}`,
          body: `Your Neural Network Lab received ${tier.label} (${Math.round(tier.value * 100)}%)`,
          icon: "🧪",
          courseId,
        });
      } catch { /* non-critical */ }
  ...
  const handleReset = async (uid) => {
    ...
    if (!window.confirm(`Reset ${name}'s Neural Network Lab? This will delete their submission and let them redo it.`)) return;
    try {
      if (sub) await deleteDoc(doc(db, "neural_network_lab", sub.id));
      const courseId = grades[uid]?.courseId || parentCourseId;
      await deleteDoc(doc(db, "progress", uid, "courses", courseId, "activities", "neural-network-lab"));
    ...
    <div>
      ...
      {submissions.length === 0 ? (
        <div className="card" style={{ textAlign: "center", padding: 48, color: "var(--text3)" }}>
          ...
          <p style={{ fontSize: 15, fontWeight: 600 }}>No Neural Network Lab submissions yet</p>
