...
const PHASES = [
  ...
  { num: 3, label: "Intent Classifier", icon: "🧠", color: "var(--purple)", description: "Train AI to understand intent" },
        ...
        <div className="bb-editor-content">
          ...
          {activePhase === 3 && (
            <IntentClassifierEditor
              config={project.phases?.[3] || { intents: [], trainingData: [], fallbackResponse: "", confidenceThreshold: 0.65 }}
              onSave={config => savePhaseConfig(3, config)}
              getToken={getToken}
            />
          )}
          {activePhase === 4 && (
