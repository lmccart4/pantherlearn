// seed-phys-u1-l11-storing-energy-v3.cjs — Unit 1 Lesson 11: Storing Energy (expanded).
// v3 = v2 lesson plus appended native graded practice blocks from l11-storing-energy-practice-sheet.md.
...
// Run: cd ~/pantherlearn-wt/openscied-u1-grid-sim && NODE_PATH=~/pantherlearn/node_modules node scripts/seed-phys-u1-l11-storing-energy-v3.cjs
...
const lesson = {
  ...
  blocks: [
    ...
    // ----- Appended native practice blocks from l11-storing-energy-practice-sheet.md -----
    k.sectionHeader({ icon: '🧮', title: 'Practice: Stored Energy Calculations', subtitle: 'Apply $PE = mgh$ and compare storage options' }),

    k.shortAnswer({
      prompt:
        'A pumped-hydro reservoir holds $8{,}000\\text{ kg}$ of water $120\\text{ m}$ above the turbines.\n' +
        '1. Calculate the stored gravitational potential energy in joules. Use $PE = mgh$ and $g = 9.8\\text{ m/s}^2$.\n' +
        '2. Convert that energy to kilowatt-hours ($1\\text{ kWh} = 3.6\\times10^6\\text{ J}$).\n' +
        '3. If a small school uses $40\\text{ kW}$ of power at peak, how many hours could this stored energy run the school?',
      placeholder: '$PE = $ ___ J\n$PE = $ ___ kWh\nRun time = $ $ ___ h',
      difficulty: 'apply',
    }),

    k.shortAnswer({
      prompt:
        'A grid-scale battery bank is rated at $500\\text{ kWh}$ of storage.\n' +
        '1. How many hours could it supply $50\\text{ kW}$ of power?\n' +
        '2. A single home uses about $1\\text{ kW}$ on average during the evening. How many homes could this battery power for one hour?',
      placeholder: 'Hours at 50 kW: ___\nHomes for one hour: ___',
      difficulty: 'apply',
    }),

    k.mc({
      prompt:
        'If both systems store the same amount of energy, why might an engineer still choose the battery bank?',
      options: [
        'Batteries respond fast and do not need hills or water.',
        'Batteries usually cost less per kilowatt-hour than pumped-hydro.',
        'Batteries can store energy for weeks without losing it.',
        'Batteries use water and gravity to store energy.',
      ],
      correctIndex: 0,
      explanation:
        'Batteries can be placed almost anywhere and respond in milliseconds, making them useful for sudden demand spikes even when they cost more per stored kilowatt-hour.',
      difficulty: 'apply',
    }),

    k.mc({
      prompt:
        'Why might an engineer choose pumped-hydro storage instead of batteries?',
      options: [
        'Pumped-hydro responds faster than batteries.',
        'Pumped-hydro can be built on flat land with no water source.',
        'Pumped-hydro is portable and fits inside a phone.',
        'Pumped-hydro often has lower cost per stored kWh.',
      ],
      correctIndex: 3,
      explanation:
        'Pumped-hydro typically has lower cost per kilowatt-hour at large scale, but it needs suitable geography and takes longer to start than batteries.',
      difficulty: 'apply',
    }),
  ],
};
