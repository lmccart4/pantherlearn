// seed-phys-u1-l07-grid-system-v3.cjs — Unit 1 Lesson 7: the grid as a system (supply must match demand).
// v3 = v2 lesson plus appended native graded practice blocks from l07-load-curve-worksheet.md.
// Run: cd ~/pantherlearn-wt/openscied-u1-grid-sim && NODE_PATH=~/pantherlearn/node_modules node scripts/seed-phys-u1-l07-grid-system-v3.cjs
...
const lesson = {
  ...
  blocks: [
    ...
    // ----- Appended native practice blocks from l07-load-curve-worksheet.md -----
    k.sectionHeader({ icon: '📈', title: 'Practice: Match Generation to Demand', subtitle: 'Use the worked-example load curve and source mix' }),

    k.mc({
      prompt:
        'At 3 AM, demand is $3{,}500\\text{ MW}$. The baseload nuclear plant supplies $3{,}000\\text{ MW}$ and the coal plant supplies $2{,}000\\text{ MW}$. Which sources must be running?',
      options: [
        'Solar only',
        'Nuclear + coal + natural-gas peaker',
        'Natural-gas peaker only',
        'Nuclear + coal only',
      ],
      correctIndex: 3,
      explanation:
        'Baseload plants run all day. Nuclear ($3{,}000\\text{ MW}$) plus coal ($2{,}000\\text{ MW}$) already exceeds the $3{,}500\\text{ MW}$ demand, so the peaker can stay off.',
      difficulty: 'apply',
    }),

    k.mc({
      prompt:
        'At noon, demand is $7{,}000\\text{ MW}$. Baseload plants supply $5{,}000\\text{ MW}$ and solar produces $2{,}000\\text{ MW}$. How much must the natural-gas peaker plant produce?',
      options: [
        '$2{,}000\\text{ MW}$ of peaker output',
        '$1{,}000\\text{ MW}$ of peaker output',
        '$0\\text{ MW}$ of peaker output',
        '$4{,}000\\text{ MW}$ of peaker output',
      ],
      correctIndex: 2,
      explanation:
        'Baseload ($5{,}000\\text{ MW}$) plus solar ($2{,}000\\text{ MW}$) equals the $7{,}000\\text{ MW}$ demand, so the peaker is not needed at noon.',
      difficulty: 'apply',
    }),

    k.mc({
      prompt:
        'At 6 PM, demand is $9{,}000\\text{ MW}$ and solar output is $0\\text{ MW}$. Baseload plants supply $5{,}000\\text{ MW}$. How much must the natural-gas peaker produce?',
      options: [
        '$4{,}000\\text{ MW}$',
        '$5{,}000\\text{ MW}$',
        '$2{,}000\\text{ MW}$',
      ...
      correctIndex: 0,
      explanation:
        'Demand ($9{,}000\\text{ MW}$) minus baseload ($5{,}000\\text{ MW}$) leaves $4{,}000\\text{ MW}$ for the peaker, which is its maximum output.',
      difficulty: 'apply',
    }),

    k.shortAnswer({
      prompt:
        'Explain why solar panels alone cannot meet the 6 PM peak in this example.',
      placeholder: 'Solar panels alone cannot meet the 6 PM peak because…',
      difficulty: 'apply',
    }),

    k.shortAnswer({
      prompt:
        'Name one advantage and one disadvantage of relying on natural-gas peaker plants to handle demand peaks.',
    ...
    k.shortAnswer({
      prompt:
        'Name one way batteries or other storage could help balance this grid.',
      placeholder: 'Batteries or storage could help by…',
      difficulty: 'apply',
    }),
  ],
};
