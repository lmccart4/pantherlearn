...
const RECIPEBOT_FUNCTION_URL =
  "https://us-central1-pantherlearn-d6f7c.cloudfunctions.net/queryRecipeBot";

export async function queryRecipeBot({ prompt, selectedSources, cleaningDecisions }) {
  const user = auth.currentUser;
  if (!user) throw new Error("Not authenticated");

  const authToken = await user.getIdToken();

  const response = await fetch(RECIPEBOT_FUNCTION_URL, {
    method: "POST",
    headers: {
      "Content-Type": "application/json",
      Authorization: `Bearer ${authToken}`,
    },
    body: JSON.stringify({ prompt, selectedSources, cleaningDecisions }),
  });
